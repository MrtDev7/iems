          ...
          <b-tbody>
            <b-tr v-for="Account in Accounts" :key="Account.id">
              ...
              <b-td class="font-size-sm text-center">
                <span
                  :class="
                    Account.status == 'running' ? 'bg-success' : 'bg-warning'
                  "
              ...
              <b-td class="font-size-sm text-center">
                {{ Account.server_type.name }}
              </b-td>
              <b-td class="font-size-sm text-center">
                {{ Account.datacenter.name }}
              </b-td>
              <b-td class="font-size-sm text-center">
                {{ Account.created }}
              </b-td>
...
export default {
  ...
  methods: {
    ...
    deleteAccount: function (AccountId) {
      const url = "https://api.hetzner.cloud/v1/servers/" + AccountId;
      ...
      this.swalConfirm(
        "are you sure!",
        "you are going to delete account " + AccountId,
        () => {
          axios
            .delete(url, {
              headers: headers,
    ...
    fetchAccounts: function () {
      const url =
        "https://api.hetzner.cloud/v1/servers?page=" + this.currentPage;
      ...
      axios
        ...
        .then((response) => {
          this.Accounts = [];
          this.Accounts = response.data.servers;
